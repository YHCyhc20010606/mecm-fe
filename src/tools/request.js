...
let apm = {
  ...
  confirmToDistribute (params, knowEdge) {
    if (knowEdge) {
      return POST(apmApi + '/tenants/' + getUserId() + '/packages', params)
    } else {
      return POST(apmApi + '/tenants/' + getUserId() + '/intent', params)
    }
  },
  getDistributionList () {
...
let appo = {
  ...
  getInstanceListByHost (hostIp) {
    return GET(appoApi + '/tenants/' + getUserId() + '/mechosts/' + hostIp + '/app_instance_infos')
  },
...
let check = {
  healthCheck () {
    return axios.get(healthApi + '/mechosts/checkall', {
      headers: {
        'Content-Type': 'application/json',
        'access_token': sessionStorage.getItem('accessToken')
      }
