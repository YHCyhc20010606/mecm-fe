...
export default {
  ...
  methods: {
    ...
    unpauseInstance (row) {
      ...
      this.handleOperatorVM(_hostIp, row, _param)
    },
    suspendInstance (row) {
      let _hostIp = sessionStorage.getItem('hostIp')
      let _param = {
    ...
    handleOperatorVM (_hostIp, row, _param) {
      appo.operateVM(_hostIp, row.id, _param).then(res => {
      ...
      appo.queryVMsByMechost(_hostIp).then(res => {
        ...
        if (this.currentSortOrder !== null) {
          let _column = {
            ...
            prop: this.currentSortProp
          }
          this.sortAction(_column)
        }
        this.dataLoading = false
      }).catch(() => {
        this.dataLoading = false
    ...
    sortAction (column) {
      ...
      this.paginationData.sort((a, b) => {
        ...
        if (column.order === 'ascending') {
          ...
          _res = _tempA.toLowerCase().substring(0, 1) > _tempB.toLowerCase().substring(0, 1) ? 1 : -1
          return _res
